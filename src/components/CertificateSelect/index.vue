<template>
  ...
  <script>
  import { fetchCertificates } from '@/api/certificate' // 假定的API调用
  ...
  export default {
    ...
    methods: {
      fetchCertificates() {
        ...
        fetchCertificates({ pageNum: 1, pageSize: 1000 }).then(response => {
          this.certificateList = response.data.records || [];
        });
      },
      handleCertificateChange(selectedIds) {
        const selectedCertificates = this.certificateList.filter(cert => selectedIds.includes(cert.id));
